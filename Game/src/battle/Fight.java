...
public class Fight {
	...
	private User player;
	private Monster npc;
	private boolean playerWin, playerDie, flee;
	private int exp, turns, startTurn, newDmg;

	public Fight(User player, Monster first, Engine e) {
		this.player = player;
		this.npc = first;
		this.e = e;
		turns = 0;
	...
	public String battle() {
		...
		if (npc.getName().equals("Dummy")) {
			...
			System.out.println();
			System.out.println("Player" + " VS " + npc.getName());
			System.out.println();
			...
			while (true) {
				...
				if (option.toLowerCase().equals("attack") || option.equals("a")) {
					System.out.println();
					attack(2);
					System.out.println();
			...
			while (true) {
				...
				if (option.toLowerCase().equals("attack") || option.equals("a")) {
					...
					System.out.println();
						...
						|| option.equals("i")) {
					ArrayList<Item> invo = e.fightInvo();
					if (invo.size() == 0) {
				...
				if (playerWin) {
					...
					System.out.println(npc.getName()
							...
							+ player.addGold((int) ((npc.getLevel() / player
									.getLevel()) * 10)) + " gold");
					player.addGold((int) ((npc.getLevel() / player.getLevel()) * 10));
					player.setHealth(100);
					return "You Won!";
				} else {
					return "You Died...";
	...
	void attack(int attacker) {
		...
		if (!npc.getName().equals("Dummy")) {
			...
			playerDmg = (int) ((player.getLevel() * player.getDmg() * 3) + newDmg);
			int playerRng = playerDmg / 2;
			playerDmg = playerDmg + range(playerRng);
			npcDmg = 0;
		}
